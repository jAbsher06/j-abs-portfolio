const userData = {
  githubUsername: "jAbsher06",
  name: "Jared Absher",
  designation: "Full Stack Software Engineer",
  avatarUrl: "/avatar.jpeg",
  email: "jared.absher@gmail.com",
  address: "Charlotte, NC",
  projects: [
    {
      title: "Jared's Portfolio",
      link: "https://jAbsher.dev",
      imgUrl: "/portfolio.png",
    },
    {
      title: "Fashion Factory",
      link: "https://github.com/CP-Front-End-Capstone/fashion-factory",
      imgUrl: "/fashionFactory.png",
    },
    {
      title: "Salut",
      link: "https://github.com/blue-nutrition/blue-nutrition",
      imgUrl: "/salut.png",
    },
    {
      title: "Walking in the Woods",
      link: "https://github.com/jAbsher06/hikingInTheWoods",
      imgUrl: "/algochurn.png",
    },
  ],
  about: {
    title:
      "I'm a software engineer that loves developing web applications that make a positive impact in people's lives",
    description: [
      `I'm a former Combat Medic/Paramedic with 15 years of solving challenging problems, often with limited information in dynamic environments.`,
      ``,
      ``,
    ],
    currentProject: "myPortfolio",
    currentProjectUrl: "https://jAbsher.dev",
  },
  experience: [
    {
      title: "Freelance Software Developer",
      company: "Upscale Digital Compositions",
      year: "2021",
      companyLink: "https://mroads.com",
      desc: "Redesigned website for local construction company.",
    },
    {
      title: "Graduation",
      company: "Hack Reactor",
      year: "2021",
      companyLink: "https://galvanize.com",
      desc: "Completed Advanced Software Engineering Bootcamp.",
    },
    {
      title: "Paramedic",
      company: "Mecklenburg EMS Agency",
      year: "2017",
      companyLink: "https://medic911.com",
      desc: "Provided live-saving medical treatment to those who called 911 in Charlotta, NC.",
    },
    {
      title: "Project Manager",
      company: "US Navy",
      year: "2014",
      desc: "Managed medical readiness for base in Sasebo, Japan.",
    },
    {
      title: "Medic Supervisor",
      company: "United States Navy",
      year: "2009",
      desc: "Deployed with Marine Infantry unit twice to Afghanistan.",
    },
    {
      title: "Joined Navy",
      company: "United States Navy",
      year: "2006",
      desc: "Began my career in medicine as a US Navy Medic",
    },
  ],
  resumeUrl:
    "https://docs.google.com/document/d/1QMnf6HYq_FXlCKt56-gvhdD1nxSIQlwAR72MreBeO9w/edit?usp=sharing",
  socialLinks: {
    linkedin: "https://linkedin.com/in/jaredCodes",
    github: "https://github.com/jAbsher06",
  },
};

export default userData;